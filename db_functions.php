...
//echo $query;
   ...
   public function getDriverName($driver_id)
   {
	...
	public function setSettleSale($inputs){//echo '000'; die;
		...
		$delivery_recover=$inputs['delivery_recover'];
		...
		$total_vat=$inputs['total_vat'];
		$expense=$inputs['expense'];
		$pay_back=$inputs['pay_back'];
		$loacal_purchase=$inputs['loacal_purchase'];
		...
		cg_recover,gross_total,discount,net_total,cash_drawer,settle_date,local_purchase,expense,total_vat,delivery_recover,pay_back) 
		...
		'$cg_recover','$gross_total','$discount','$net_total','$cash_drawer','$settle_date','$loacal_purchase','$expense','$total_vat','$delivery_recover','$pay_back')";
			//echo $sql; die;
			...
			if($result){
				...
				//$d_sale = $net_total;
				//$sql1 = "INSERT INTO ".DB_PRIFIX."sales (`sal_company`, `sal_branch`, `sal_date`, `sal_cash_amount`, `sal_card_amount`, `tot_vat`, `date_added`, `date_updated`) VALUES ('$company_id', '$shop_id', '$settle_date1', '$net_total', '0', '$total_vat', '$date_added', '$date_updated')";
				...
				try {
					...
					$mail->setFrom('cakegallerymail@gmail.com', $inputs['shop_name'].' POS WEB');
					$mail->addAddress('anandbe13@gmail.com', $inputs['shop_name'].' POS WEB');
					$mail->AddReplyTo('cakegallerymail@gmail.com', $inputs['shop_name'].' POS WEB');
...
// 	print_r($last_settle_sale);exit;
		...
		$loacal_purchase_db = $this->getLocalPurchase($inputs);
		
		$grand_total_tax=0;
		$grand_total_without_tax =0;
		
		$Grand_total=0;$Net_total=0;$CashSale=0;$CardSale=0;$CreditSale=0;$CashDrawerAmt=0;$CashSale1=0;$total1=0;$CardSale1=0;$CreditSale1=0;$DeliverySale1=0;$py_back_tax=0;$py_back1=0;
		$cod_pending=false;
		$hold_pending=false;
		$CashStartAmt=0;$DeliverySale=0;$CreditRecover=0;$DiscountTotal=0;$Cg_adv=0;$Cg_recover=0;$py_back=0;$add_amt_new=0;$DeliveryRecover=0;$adv_amt_new=0;$Online_Order_Recovery=0;$expense=0;$loacal_purchase=0;
		...
		if(!empty($pay_back)) {
			foreach($pay_back as $py_back_row){
				...
				$py_back1 += ($py_back_row['amount'] * $py_back_row['qty']);
			}
		}
		...
		if(!empty($orders)) {
			foreach($orders as $row){
				...
				if(count($items)!=0){
					...
					$grand_total_without_tax +=$total;
					//VAT START
					if(BILL_TAX == 'yes' && BILL_COUNTRY == 'UAE') {
						$total = $total+($total/100 * $row['vat']);
						$total1 = $total1+($total11/100 * $row['vat']);

						$grand_total_tax +=($total/100 * $row['vat']); 		            
					...
					if($row['payment_type'] == 'card'){
						...
						//$Card_vat=($CardSale1+$total1) - $row['discount'];
						//$card_sale_with_discount = $CardSale - $row['discount'];
					}
					...
					if($row['payment_type'] == 'credit'){
						...
						//$CreditSale1=($CreditSale1+$total1) - $row['discount'];
					} 
					if($row['payment_type'] == 'cod'){
						$DeliverySale=($DeliverySale+$total) - $row['discount'];						
						//$DeliverySale1=($DeliverySale1+$total1) - $row['discount'];
		...
		if(!empty($expense_db)){
		//echo '<pre>'; print_r($expense_db); die;
			foreach($expense_db as $expen){
				$expense += $expen['net_total'];
			}
		}
		if(!empty($loacal_purchase_db)){
		//echo '<pre>'; print_r($expense_db); die;
			foreach($loacal_purchase_db as $lc_pur){
				$loacal_purchase += $lc_pur['net_total'];
			}
		}
		$py_back = $py_back + ($py_back/100)*BILL_TAX_VAL;
		$py_back_tax = ($py_back1/100)*BILL_TAX_VAL;
		$Cg_recover=$add_amt_new-$adv_amt_new;
		
		//$Grand_total=$CashSale+$CardSale+$CreditSale+$CreditRecover+$DeliveryRecover+$Cg_adv+$Cg_recover+$DiscountTotal;
		$Grand_total=$CashSale+$CardSale+$CreditSale+$DeliverySale+$DiscountTotal;		
		...
		$CashDrawerAmt = $CashDrawerAmt - $expense - $loacal_purchase;
		//VAT START
		/*if(BILL_TAX == 'yes' && BILL_COUNTRY == 'UAE') {
		...
		$result_ar['gross_total_tax']=$grand_total_tax;
		//$result_ar['gross_total_without_tax']=($grand_total_without_tax + $CreditRecover);
		$result_ar['gross_total_without_tax']=($grand_total_without_tax );
		...
		$result_ar['loacal_purchase']=$loacal_purchase;
		$result_ar['cash_sale1']=$CashSale1;
		$result_ar['total_vat']=($total1-$py_back_tax);
...
public function setPayBack($inputs){
		...
		$qty=$inputs['qty'];
		$weight=$inputs['weight'];
		$lose_item=$inputs['lose_item'];
		$sale_order_item_id=$inputs['sale_order_item_id'];
		$amount=$inputs['amount'];
		$sale_order_id=$inputs['sale_order_id'];
		$payback_date=$inputs['payback_date'];
		$query="INSERT INTO ".DB_PRIFIX."pay_back(shop_id,sale_order_item_id,user_id,receipt_id,item_id,qty,amount,payback_date,sale_order_id,weight,lose_item) 
		VALUES('$shop_id','$sale_order_item_id','$user_id','$receipt_id','$item_id','$qty','$amount','$payback_date','$sale_order_id','$weight','$lose_item')";
		//echo $query; die;
		$result=mysqli_query($GLOBALS['conn'], $query);
		   if ($result) {
			   ...
			   //Return qty add in stock
			   if($lose_item == '1') {
					$stock = $qty * $weight;
			   } else {
				    $stock = $qty;
			   }
				mysqli_query($GLOBALS['conn'], "UPDATE ".DB_PRIFIX."items SET stock = stock+'$stock' WHERE id = '$item_id'");
				//Return qty add in stock END
            
				$result1 = mysqli_query($GLOBALS['conn'], "SELECT * FROM ".DB_PRIFIX."pay_back WHERE id = $pay_back_id");
	...
	public function getSaleOrderItemDetailsList($inputs) {//print_r($inputs); die;
		...
		if(isset($inputs['status']) && $inputs['status'] != '' && $status_online_order == '') {			
			...
			$query .= " OR payment_status = 'unpaid'";
		}
		//For online order
		...
		if ($result) {
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				$result_ar['contact_number'] = $row['contact_number'];
				$result_ar['address'] = $row['address'];
				$result_ar['customer_trn_no'] = $row['customer_trn_no'];
				$result_ar['order_type'] = $row['order_type'];
				$result_ar['payment_type'] = $row['payment_type'];
				$result_ar['payment_status'] = $row['payment_status'];
				$result_ar['status'] = $row['status'];
	...
	public function getSaleOrderItemDetailsListByPaidDate($inputs) {
		...
		$query .= " AND status !='hold' AND status != 'reject'";
	...
	public function getSaleOdersItemDetailsListByReceipt($inputs){
		...
		if(empty($result_arr)) {
			$query_new = "SELECT * FROM ".DB_PRIFIX."sale_order_items WHERE ime_no='$receipt_id'";
			$result_new = mysqli_query($GLOBALS['conn'], $query_new);
			if ($result_new) {				
				while ($row1 = mysqli_fetch_assoc($result_new)) {					
					$sale_order_id = $row1['sale_order_id'];
				}
				$result_arr = array();
				$query = "SELECT * FROM ".DB_PRIFIX."sale_orders where id='$sale_order_id'";
				$result = mysqli_query($GLOBALS['conn'], $query);
				if ($result) {
					$result_arr = array();
					while ($row = mysqli_fetch_assoc($result)) {
						...
						$order_items_arr = array();
						$query2 = "SELECT * FROM ".DB_PRIFIX."sale_order_items WHERE ime_no='".$receipt_id."'";						
						$result2 = mysqli_query($GLOBALS['conn'], $query2);
						if ($result2) {
							while ($row2 = mysqli_fetch_assoc($result2)) {						
								$query3 = "SELECT * FROM ".DB_PRIFIX."pay_back WHERE receipt_id='".$receipt_id."'";
								$result3 = mysqli_query($GLOBALS['conn'], $query3);
								if ($result3) {
									while($row3 = mysqli_fetch_assoc($result3)){
										$total_rem = ($row2['qty'] - $row3['qty']);	
	...
	public function getExpense($inputs){
		...
		if((isset($from_date) && $from_date != '') && (isset($to_date) && $to_date != '' )){
			$query .=" WHERE user_id='$user_id' AND shop_id='$shop_id' AND date_added BETWEEN '$from_date' AND '$to_date'";
		}else{
			$query .=" where user_id='$user_id' AND shop_id='$shop_id'";
		}
		//echo $query; die;
...
*/
	public function getExpense($inputs){
		...
		$from_date=$inputs['from_date'];
		$to_date=$inputs['to_date'];
		$query="SELECT sub_total,vat_amount,net_total from expense";
		if((isset($from_date) && $from_date != '') && (isset($to_date) && $to_date != '' )){
			//$query .=" WHERE user_id='$user_id' AND exp_company='$shop_id' AND date_added BETWEEN '$from_date' AND '$to_date'";
			$query .=" WHERE  date_added BETWEEN '$from_date' AND '$to_date'";
		}else{
			//$query .=" where user_id='$user_id' AND exp_company='$shop_id'";
		}
		//echo $query; die;
		$result=mysqli_query($GLOBALS['conn'], $query);
		if($result){
	...
	public function getLocalPurchase($inputs){
		$user_id=$inputs['user_id'];
		$shop_id=$inputs['shop_id'];
		$from_date=$inputs['from_date'];
		$to_date=$inputs['to_date'];
		$query="SELECT net_total from ".DB_PRIFIX."local_purchase_front";
		if((isset($from_date) && $from_date != '') && (isset($to_date) && $to_date != '' )){
			$query .=" WHERE user_id='$user_id' AND shop_id='$shop_id' AND date_added BETWEEN '$from_date' AND '$to_date'";
		}else{
			$query .=" where user_id='$user_id' AND shop_id='$shop_id'";
