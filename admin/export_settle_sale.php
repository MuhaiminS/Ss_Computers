...
$table .= '<table border="1" cellspacing="0" bordercolor="#222"><tr>';
	$table .= '<th style="background-color:#244062; color:#fff;">Settle Sale Reports</th></tr>';
	$table .= '<tr><th style="background-color:#244062; color:#fff;">From Date: '.$from_date.'</th></tr>';
	$table .= '<tr><th style="background-color:#244062; color:#fff;">To Date: '.$to_date.'</th>';
$table .= '</tr><table>'; 
...
$table .= '<table border="1" cellspacing="0" bordercolor="#222"><tr>'; 
		...
		$table .= '<td style="background-color:#244062; color:#fff;">Credit Sale</td>';  
  ...
  $grand_total = $total_vat1 = $cash_sale_total = $delivery_sale_total =  $card_sale_total= $credit_sale_total  = $sale_total= $discount_total="0.00";												
	...
	if($prs != false) {
		...
		if($pcount > 0) {
			for($p = 0; $p < $pcount; $p++) {
				...
				$credit_sale = ($prow->credit_sale !='') ? $prow->credit_sale :  '0.00' ;
				...
				$total = "0.00";
				$total_vat = ($prow->total_vat !='') ? $prow->total_vat :  '0.00' ;
			...
			$table .=  "<td>".number_format($cash_sale, 2)."</td>";
			$table .=  "<td>".number_format($card_sale, 2)."</td>";
			$table .=  "<td>".number_format($credit_sale, 2)."</td>";
			$table .=  "<td>".number_format($delivery_sale, 2)."</td>";
			$table .=  "<td>".number_format($total_without_discount, 2)."</td>";
			$table .=  "<td>".number_format($discount, 2)."</td>";
			$table .=  "<td>".number_format($total_vat, 2)."</td>";
			$table .=  "<td style='background-color:#ffff00;'>".number_format(($total_without_discount-$discount), 2)."</td>";
			$table .=  "</tr>";
			
			$cash_sale_total += $cash_sale;
			$card_sale_total += $card_sale;
			$credit_sale_total += $credit_sale;
			$delivery_sale_total += $delivery_sale;
			$sale_total += $total_without_discount;
			$discount_total += $discount;
			$total_vat1 += $total_vat;
			$grand_total += $total_without_discount-$discount;
		}
		$table .=  "<tr>";
		$table .=  "<td colspan='2'>Final balance</td>";		
		$table .=  "<td style='background-color:#ffff00;'>".number_format($cash_sale_total, 2)."</td>";
		$table .=  "<td style='background-color:#ffff00;'>".number_format($card_sale_total, 2)."</td>";
		$table .=  "<td style='background-color:#ffff00;'>".number_format($credit_sale_total, 2)."</td>";
		$table .=  "<td style='background-color:#ffff00;'>".number_format($delivery_sale_total, 2)."</td>";
		$table .=  "<td style='background-color:#ffff00;'>".number_format($sale_total, 2)."</td>";
		$table .=  "<td style='background-color:#ffff00;'>".number_format($discount_total, 2)."</td>";
