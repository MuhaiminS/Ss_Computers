...
if(isset($_POST['items_post'])) {
	...
	$weight = $_POST['weight'];
	$lose_item = isset($_POST['lose_item'])? $_POST['lose_item']:'0';
...
//	$inward_date = $_POST['inward_date'];
	...
	if(isset($_POST['id']) && $_POST['id'] > 0) {
		$id = $_POST['id'];		
		$qry = "UPDATE items SET lose_item = '$lose_item', 
									cat_id = '$cat_id', 
									barcode_id= '$barcode_id', 
									name = '".safeTextIn($name)."', 
									price = '".safeTextIn($price)."', 
									cost_price = '".safeTextIn($cost_price)."', 
									weight = '".safeTextIn($weight)."', 
									unit = '".safeTextIn($unit)."' 
									WHERE id = '$id'";
		//	echo $qry; die;
		...
		if(mysqli_num_rows($result_rad)) {
			...
			$stock = $_POST['stock'];
			$qry = "INSERT INTO items (cat_id, name, price, weight, unit, barcode_id, stock, lose_item,cost_price) VALUES ('$cat_id', '".safeTextIn($name)."', '".safeTextIn($price)."', '".safeTextIn($weight)."', '".safeTextIn($unit)."', '".safeTextIn($barcode_id)."', '$stock', '$lose_item','".safeTextIn($cost_price)."')";
			//echo $qry; die;
			if(mysqli_query($GLOBALS['conn'], $qry)){		
	...
	if (file_exists($_FILES['image']['tmp_name'])) {		
		...
		//die;
	redirect('product.php?resp=addsucc');
}
...
if(isset($_GET['act']) && $_GET['act'] != '' && isset($_GET['id']) && $_GET['id'] > 0) {	
	...
	if($action == 'edit') {
		...
		while ($edit_row = mysqli_fetch_array($run_edit)) {
			...
			$cost_price = $edit_row['cost_price'];
			//$sgst = $edit_row['SGST'];
			$image = $edit_row['image'];
			$lose_item = $edit_row['lose_item'];
				...
				<section class="content container-fluid">
					...
					<div class="box box-default">
						...
						<div class="box-body">
							<div class="row">
								<div class="col-md-6 ">
									<form action="product_add.php" method="post" id="defaultForm" enctype="multipart/form-data">
										...
										<div class="form-group" id="cke_editor1">
			...
			//        live: 'disabled',
			      ...
			      fields: {
			        ...
			        /*  username: {
					  ...
					  ingredient_stock_reduce: {
			              validators: {
			                  notEmpty: {
			                      message: 'Please Select Ingredient Stock Reduce'
			                  }
			              }
			          },
			     /*     'languages[]': {
