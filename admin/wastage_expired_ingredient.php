<?php 
	...
	function getExpiredIngredient($from_date ='', $to_date='', $export=""){					

		$qry="SELECT * FROM purchase_order_items WHERE 1"; 
	
		/*if($from_date != '' && $to_date != '') {
			$qry .= " AND expiry_date BETWEEN '$from_date 00:00:00' AND '$from_date 23:59:59' ";
		}*/
		if($from_date != '') {
			$qry .= " AND expiry_date <= '$from_date'";
		}
			...
			<div class="content-wrapper">
				...
				<section class="content-header">
					...
					<ol class="breadcrumb">
						...
						<li class="active">Expired Ingredient</li>
					</ol>
				</section>
				<!-- Main content -->
						...
						<div class="col-xs-12">
							<div class="box">
								<div class="box-header">
									...
									<h3 class="box-title">Expired Ingredient</h3>
								</div>
								<!-- /.box-header -->
								<div class="box-body">
									<div id="form" class="panel panel-warning" style="display: block;">
										<div class="panel-body">
											<form action="wastage_expired_ingredient.php" accept-charset="utf-8">
												<div class="row">
													<div class="col-sm-3">
													...
													<div class="col-sm-12">
														...
														<a href="wastage_expired_ingredient.php" class="btn btn-default">Reset</a>
													</div>
												</div>
											</form>
										</div>
									...
									<table id="example2" class="table table-bordered table-hover">
										...
										<tbody>
											...
											$i=$grand_total = $total_qty=0;		
												$prs = getExpiredIngredient($from_date1, $to_date1, $export="");									
												if($prs != false) {
													$pcount = mysqli_num_rows($prs);
													if($pcount > 0) {
														for($p = 0; $p < $pcount; $p++) {
															$prow = mysqli_fetch_object($prs);														
															$items_name = $prow->items_name;
															$qty  = $prow->qty;
															//$total  = $prow->amount;
															$weight  = $prow->weight;
															$incrediant_unit  = $prow->incrediant_unit;
											...
											<tr>
												...
												<td><?php echo $incrediant_unit; ?></td>
											...
											<?php
												...
												echo "<tr>";
												echo "<td>No Ingredient found to list.</td>";
												echo "</tr>";
												}?>									
										</tbody>										
									</table>
