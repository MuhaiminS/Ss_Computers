...
function getUsers()
{
	$qry="SELECT U.id AS user_id, R.title, U.* FROM users AS U LEFT JOIN users_role AS R ON U.role_id = R.id WHERE U.role_id != 1 ORDER BY U.id ASC";
	//echo $qry;
	...
	<body class="hold-transition skin-blue sidebar-mini">
											...
											if($prs != false) {
												...
												if($pcount > 0) {
													for($p = 0; $p < $pcount; $p++) {
														...
														echo "<td>";
															echo "<a href='users_add.php?id=".$user_id."&act=edit'  class='tip btn btn-primary btn-xs'><i class='fa fa-edit'></i></a>";
		...
		<script type="text/javascript">
			...
			function deleteIt(id)
			{
				...
				{
					window.location.href = site_url+'admin/users.php?id='+id+'&act=delete';
				}
			}
			function changeStatus(status, id)
			{
				...
				if(id && confirm(msg))
				{
					window.location.href = site_url+'/admin/users.php?id='+id+'&act='+status;
				}
			}
		</script>
		</body>
